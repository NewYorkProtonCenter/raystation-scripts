...
def create_expanded_roi(pm, examination, ss, roi):
  ...
  if roi_geometry and SSF.has_roi(ss, roi.source.name):
    ...
    print('{roi.source.name} is missing')
...
def create_external_geometry(pm, examination, ss):
  #JZ changed: don't delete the External. If you find it, just no need to created a new one
  HasExternal = False #JZ added
  for pm_roi in pm.RegionsOfInterest:
    if pm_roi.Type == 'External':
      HasExternal = True
      ss.RoiGeometries[pm_roi.Name].OfRoi.CreateExternalGeometry(Examination = examination, ThresholdLevel = None)
    # pm_roi.DeleteRoi()
      break
  if not HasExternal:
