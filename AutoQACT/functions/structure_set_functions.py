# encoding: utf8
# 2021.11.15: changed find_OARs(). Now does not include the empty OARs and ROIs with density override
# 2022.03.05: added find_IGRT_Surface().
...
# JZ added: Find all target volume in the exam.
def find_CTVs(ss):
  target = []
  # CTV based on total dose, for example CTV_50
  for roi in ss.RoiGeometries:
    ##no yyyy.mm.dd in the name, no opt, Opt, Opti, etc. in the name
    if roi.OfRoi.OrganData.OrganType=='Target' and roi.HasContours()\
      and (re.search(r'op', roi.OfRoi.Name, re.IGNORECASE) is None)\
      and (re.search(r'z_', roi.OfRoi.Name, re.IGNORECASE) is None)\
...
def find_Biology_OARs(ss):
  target = []
  OARs = []
  for roi in ss.RoiGeometries:
    if roi.OfRoi.OrganData.OrganType=='Target' and roi.HasContours() and roi.OfRoi.Name.find('TCP') != -1:
      target.append(roi.OfRoi.Name)
    if roi.OfRoi.OrganData.OrganType=='Organ' and roi.HasContours() and roi.OfRoi.Name.find('NTCP') != -1:
      OARs.append(roi.OfRoi.Name)
  return target, OARs

# JZ added: Find all organs and body
def find_OARs(ss):
  OARs = []
  for roi in ss.RoiGeometries:
    if roi.HasContours():
      try:
        overridematerial = roi.OfRoi.RoiMaterial.OfMaterial.Name
      except:#do not include any ROI with density override
        if roi.OfRoi.Type == 'Organ' and roi.HasContours():
          OARs.append(roi.OfRoi.Name)
        elif roi.OfRoi.Name.lower() == 'body' and roi.HasContours():
          OARs.append(roi.OfRoi.Name)
        # elif roi.OfRoi.Name == 'Body' and roi.HasContours():
        #   OARs.append(roi.OfRoi.Name)
        # elif roi.OfRoi.Name == 'BODY' and roi.HasContours():
        #   OARs.append(roi.OfRoi.Name)
        else:
  ...
  if len(OARs) == 0:
    print('OARs is missing')

  return OARs

# JZ added: Find external and all supporting structure
def find_External_Support(ss):
  ext_sup_ROIs = []
  for roi in ss.RoiGeometries:
    if (roi.OfRoi.Type == 'External' or roi.OfRoi.Type == 'Support') and roi.HasContours():
      ext_sup_ROIs.append(roi.OfRoi.Name)

  return ext_sup_ROIs

# JZ added: Find external structure
def find_External(ss):
  ext_ROI = ''
  for roi in ss.RoiGeometries:
    if (roi.OfRoi.Type == 'External') and roi.HasContours():
      ext_ROI = roi.OfRoi.Name
      break

  return ext_ROI

# JZ added: Find all ROIs with contour
def find_all_roi(ss):
  all_roi = []
  for roi in ss.RoiGeometries:
    if roi.HasContours():
      all_roi.append(roi.OfRoi.Name)

  return all_roi

# JZ added: Find all ROIs with density override
def find_override_ROI(ss):
  overrideROIs = []
  supportROIs = []
  for roi in ss.RoiGeometries:
    if roi.HasContours():
      try:
        overridematerial = roi.OfRoi.RoiMaterial.OfMaterial.Name
        if re.search(r'air_in_TPCT', roi.OfRoi.Name, re.IGNORECASE) is None:
          overrideROIs.append(roi.OfRoi.Name)
        if roi.OfRoi.Type == 'Support':
          supportROIs.append(roi.OfRoi.Name)
      except:
        continue

  return overrideROIs, supportROIs

#From Chih-wei
def find_body(ss):
    OARs = []
    for roi in ss.RoiGeometries:
        if roi.OfRoi.Name.lower() == 'body' and roi.HasContours():
            OARs.append(roi.OfRoi.Name)
        else:
            continue
...
# JZ added: Find IGRT_X_Surface
def find_IGRT_Surface(ss):
    OARs = []
    for roi in ss.RoiGeometries:
        if re.search(r'igrt', roi.OfRoi.Name, re.IGNORECASE) and  re.search(r'surface', roi.OfRoi.Name, re.IGNORECASE) and roi.HasContours():
            OARs.append(roi.OfRoi.Name)
        else:
            continue
